...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
    ...
    # print("INPUT: ", embedded.shape)
    outputs_flat = batchify(fn, netchunk)(embedded)
    # print("OUTPUT: ", outputs_flat.shape)
    outputs_flat[~keep_mask, -1] = 0 # set sigma to 0 for invalid points
    outputs = torch.reshape(outputs_flat, list(inputs.shape[:-1]) + [outputs_flat.shape[-1]])
    # print("OUTPUT_reshaped: ", outputs.shape)
    return outputs
...
def batchify_rays(rays_flat, chunk=1024*32, **kwargs):
    ...
    # print("bachify_rays outputs: ", all_ret['rgb_map'].shape)
    return all_ret
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    print("Inside render_path(), printing render_poses", render_poses.shape)
    H, W, focal = hwf
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        # print("Inside render_path(), printing c2w", c2w.shape, c2w, c2w[:3,:4])
        print(i, time.time() - t)
        t = time.time()
...
def render_rays(ray_batch,
    ...
    # print("output of raw2outputs rgb_map", rgb_map.shape)
    if N_importance > 0:
...
def config_parser():
    ...
    parser.add_argument("--i_weights", type=int, default=1500,
                        help='frequency of weight ckpt saving')
    parser.add_argument("--i_testset", type=int, default=1500,
...
def train():
    ...
    if use_batching: ### we are using batching
    ...
    for i in trange(start, N_iters):
        ...
        if i%args.i_video==0 and i > 0:
            ...
            def create_video_file(arr, output_file = 'output.mp4'):
        ...
        if i%args.i_testset==0 and i > 0:
            ...
            with torch.no_grad():
                print("Input for render_path()", torch.Tensor(poses[i_test]).shape)
